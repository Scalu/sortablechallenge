...
// OriginalMatcherPreFix the original matching algorithm for this challenge
type OriginalMatcherPreFix struct {
	products Products
	listings Listings
}

// GetID returns this matcher's ID
func (om *OriginalMatcherPreFix) GetID() string {
	return "originalMatcherPreFix"
}

// GetResults returns an array of results produced by the original algorithm
func (om *OriginalMatcherPreFix) GetResults(products []sortablechallengeutils.Product, listings []sortablechallengeutils.Listing) (results []sortablechallengeutils.Result) {
